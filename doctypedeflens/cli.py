...
"""doctypedeflens CLI: suites | check | validate | audit. Read-only, offline."""
...
def cmd_audit(args):
    ...
    for (skill, task), (h, n) in sorted(per_suite.items()):
        print(f"{skill:12s} {task:22s} hard={h}/{n}={h / n:.4f}")
...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="doctypedeflens")
